...
public class DealerMenu extends AbstractContainerMenu {
  ...
  // For this container, we can see both the tile inventory's slots as well as the player inventory
  // slots and the hotbar.
  ...
  private static final int ADDITIONAL_Y_OFFSET = 50;
  ...
  private void addPlayerInventory(Inventory playerInventory) {
    int offsetY = 84 + ADDITIONAL_Y_OFFSET;
    for (int i = 0; i < 3; ++i) {
      for (int l = 0; l < 9; ++l) {
        this.addSlot(new Slot(playerInventory, l + i * 9 + 9, 8 + l * 18, offsetY + i * 18));
      }
    }
  }

  private void addPlayerHotbar(Inventory playerInventory) {
    int offsetY = 142 + ADDITIONAL_Y_OFFSET;
    for (int i = 0; i < 9; ++i) {
      this.addSlot(new Slot(playerInventory, i, 8 + i * 18, offsetY));
    }
  }
}
